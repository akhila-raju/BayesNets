...
def constructBayesNet(gameState):
    ...
    # of possible assignments to `var`. These should again be set using the
    # constants defined at the top of this file.
    variableDomainsDict[X_POS_VAR] = tuple(X_POS_VALS)
    variableDomainsDict[Y_POS_VAR] = tuple(Y_POS_VALS)
    variableDomainsDict[FOOD_HOUSE_VAR] = tuple(HOUSE_VALS)
    variableDomainsDict[GHOST_HOUSE_VAR] = tuple(HOUSE_VALS)
    # variableDomainsDict[tuple(obsVars)] = tuple(OBS_VALS)
...
def fillObsCPT(bayesNet, gameState):
    ...
    house_positions = gameState.getPossibleHouses()
    ...
    for house_position in house_positions:
        ...
        for wall in gameState.getHouseWalls(house_position):

            wall = OBS_VAR_TEMPLATE % wall
            factor = bn.Factor([wall], HOUSE_VARS, bayesNet.variableDomainsDict())
            ...
            for assignment in factor.getAllPossibleAssignmentDicts():
                ...
                if assignment[FOOD_HOUSE_VAR] == house_values[house_counter]:
                    if assignment[wall] == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif assignment[wall] == BLUE_OBS_VAL:
                ...
                elif assignment[GHOST_HOUSE_VAR] == house_values[house_counter]:
                    if assignment[wall] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assignment[wall] == BLUE_OBS_VAL:
                        prob = 1 - PROB_GHOST_RED
                
                # If the adjacent house center is occupied by neither the ghost house or the food house, 
                # an observation is none with certainty (probability 1).
                else:
                    if assignment[wall] == NO_OBS_VAL:
                        prob = 1

                factor.setProbability(assignment, prob)

            bayesNet.setCPT(wall, factor)

        house_counter = (house_counter + 1) % 4
...
class VPIAgent(BayesAgent):
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, newEvidence in self.getExplorationProbsAndOutcomes(evidence):
